	...
	<td class='message_from'>
		<?php 
			...
			if(trim($ticket['number'])){
				echo '<span style="font-family:Arial, Helvetica, sans-serif; font-size:12px; font-weight:bold; color:#fff; text-decoration:none;">'.$ticket['number'].'</span>'."<br>";
			}
		?>
		</span>
	</td>
				...
				<td class='message_from'>
					<?php 
						...
						//if from reporter
							...
							echo $ticket['name']."<br>";
							if(trim($ticket['email'])){
								echo '<span style="font-family:Arial, Helvetica, sans-serif; font-size:12px; font-weight:bold; color:#fff; text-decoration:none;">'.$ticket['email'].'</span>'."<br>";
							}
							if(trim($ticket['number'])){
								echo '<span style="font-family:Arial, Helvetica, sans-serif; font-size:12px; font-weight:bold; color:#fff; text-decoration:none;">'.$ticket['number'].'</span>'."<br>";
							}
